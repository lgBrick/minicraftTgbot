...
def stream_response_to_telegram(message, prompt):
    """
    Универсальная функция для стриминга ответа в Telegram.
    Собирает текст кусками и постепенно редактирует сообщение.
    """
    # Сначала отправляем заглушку, которую будем редактировать
    ...
    try:
        ...
        for chunk in response_stream:
            if chunk.text:
                ...
                # Обновляем сообщение в ТГ не на каждое слово (чтобы не спамить апи),
                # а каждые 5 чанков (примерно 5-7 слов). Так быстрее и безопаснее.
                if chunk_counter % 5 == 0:
                    try:
                        ...
                        pass  # Игнорируем мелкие ошибки ТГ вроде "сообщение не изменилось"

        # Финальный аккорд — убираем троеточие и выводим чистый готовый текст
        bot.edit_message_text(
            chat_id=message.chat.id,
            message_id=sent_msg.message_id,
            text=current_text if current_text else "Бля, чё-то пошло не так, Gemini промолчал. 🤷‍♂️"
        )

    except Exception as e:
        # Если всё упало, пишем ошибку прямо в это же сообщение
        bot.edit_message_text(
            chat_id=message.chat.id,
            message_id=sent_msg.message_id,
...
def handle_guide_reply(message):
    ...
    4. Главная цель — всегда давать четкий, правильный и полезный ответ на вопрос пользователя. Отвечай компактно, не пиши огромные мемуары, чтобы уложиться в лимиты.
